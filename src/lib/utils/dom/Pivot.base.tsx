...
>((props, ref) => {
	...
	const isKeyValid = (itemKey: string | null | undefined): boolean => {
		return (
			itemKey === null ||
			(itemKey !== undefined && linkCollection.keyToIndexMapping[itemKey] !== undefined)
		);
	};
	...
	const renderedSelectedIndex = renderedSelectedKey
			...
			overflowMenuProps.items = linkCollection.links
				...
				.map((link, index) => {
					...
					return {
						...
						onRender: () =>
							renderPivotLink(linkCollection, link, renderedSelectedKey, classNames.linkInMenu)
					};
				});
		},
		rtl: getRTL(theme),
